...
class SummerCourseActiveRegistrationModal extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<SummerCourseActiveRegistrationModal> createState() => _SummerCourseActiveRegistrationModalState();
}

class _SummerCourseActiveRegistrationModalState extends ConsumerState<SummerCourseActiveRegistrationModal> {
  ...
  Future<void> _updateActivity(int participantId, int? newActivityId) async {
    ...
    try {
      final service = ref.read(summerCourseServiceProvider);
      final success = await service.updateIntensiveActivity(participantId, newActivityId);
      ...
      if (mounted) {
        ToastAlerts.showError(context, 'Ocurrió un error inesperado al actualizar la actividad.');
      }
    } finally {
      if (mounted) {
          ...
          child: Column(
            ...
            children: [
              ...
              ..._intensiveActivities.map((act) {
                final cost = double.tryParse(act['extra_cost']?.toString() ?? '0') ?? 0;
                return ListTile(
                  contentPadding: EdgeInsets.zero,
                  leading: Container(
                    padding: const EdgeInsets.all(8),
                    decoration: BoxDecoration(color: AppTheme.vibrantGold.withOpacity(0.1), shape: BoxShape.circle),
                    child: const Icon(Icons.star_rounded, color: AppTheme.vibrantGold),
                  ),
                  title: Text(act['name'] as String, style: const TextStyle(fontWeight: FontWeight.bold)),
                  subtitle: Text('Costo adicional: \$${cost.toStringAsFixed(2)} MXN', style: const TextStyle(color: AppTheme.successColor, fontSize: 12, fontWeight: FontWeight.w600)),
                  trailing: const Icon(Icons.chevron_right_rounded, color: AppTheme.neutral400),
                  onTap: () {
                    Navigator.pop(ctx);
                    final aId = int.tryParse(act['id']?.toString() ?? '0') ?? 0;
                    _requestUpgrade(participantId, aId, (act['name'] ?? '').toString(), cost);
                  },
                );
              }).toList(),
            ],
  ...
  Future<void> _requestUpgrade(int participantId, int activityId, String activityName, double cost) async {
    setState(() => _isSaving = true);
    ...
    try {
      ...
      if (response.statusCode == 200 && mounted) {
        ...
        if (response.data is Map && response.data['data'] is Map) {
          orderId = response.data['data']['sales_order_id']?.toString() ?? orderId;
        } else {
        ...
        showDialog(
          ...
          builder: (dCtx) => AlertDialog(
            ...
            actions: [
              TextButton(
                onPressed: () {
                  Navigator.pop(dCtx); // Mantiene el modal abierto. Se actualizará vía Riverpod.
                },
                child: const Text('Entendido', style: TextStyle(color: AppTheme.primaryColor, fontWeight: FontWeight.bold)),
              )
            ],
          ),
        );
  ...
  Widget build(BuildContext context) {
    ...
    final participants = activeRegAsync.value?['participants'] as List<dynamic>? ?? widget.registrationData['participants'] as List<dynamic>? ?? [];
            ...
            child: Row(
              children: [
                Container(
                  ...
                  child: const Icon(Icons.snowboarding_rounded, color: AppTheme.primaryColor, size: 28),
                ),
                ...
                IconButton(
                  ...
                  icon: const Icon(Icons.refresh_rounded, color: AppTheme.neutral500),
                  tooltip: 'Actualizar',
                ),
                Container(
                  decoration: BoxDecoration(
          ...
          Expanded(
            child: _isLoadingActivities
                ...
                : ListView.separated(
                    ...
                    separatorBuilder: (_, __) => Divider(height: 48, color: AppTheme.neutral200.withOpacity(0.5)),
                        ...
                        children: [
                          Row(
                            children: [
                              Container(
                                ...
                                child: const Icon(Icons.person_rounded, size: 20, color: AppTheme.neutral500),
                              ),
                          ...
                          Container(
                            ...
                            decoration: BoxDecoration(
                              ...
                              border: Border.all(
                                color: isPending ? AppTheme.warningColor.withValues(alpha: 0.5) : (Theme.of(context).brightness == Brightness.dark 
                                    ? AppTheme.neutral700 
                                    : AppTheme.neutral200),
                              ),
                            ),
                            child: isPending 
                              ? Row(
                                  children: [
                                    ...
                                    const Icon(Icons.access_time_rounded, color: AppTheme.warningColor, size: 18),
                                  ],
                                )
                              : DropdownButtonHideUnderline(
                                  child: DropdownButton<int?>(
                                    ...
                                    isDense: true,
                                    icon: canEdit 
                                        ? const Icon(Icons.unfold_more_rounded, color: AppTheme.primaryColor)
                                        : const Icon(Icons.lock_rounded, color: AppTheme.neutral400, size: 16),
                                    style: TextStyle(
                                      fontSize: 14,
                                      fontWeight: FontWeight.w700,
                                      color: canEdit 
                                    ...
                                    hint: const Text('Regular (Sin verano intensivo)'),
                                    items: [
                                      const DropdownMenuItem<int?>(
                                        value: null,
                                        child: Text('Regular (Sin verano intensivo)'),
                                      ),
                                      ..._intensiveActivities.map((act) {
                                        return DropdownMenuItem<int?>(
                                          value: act['id'] as int,
                                          child: Text(act['name'] as String, style: TextStyle(color: canEdit ? AppTheme.primaryColor : AppTheme.neutral500)),
                                        );
                                      }).toList(),
                                    ],
                            ...
                            SizedBox(
                              ...
                              child: TextButton.icon(
                                onPressed: _isSaving ? null : () {
                                  _showUpgradeActivitySelector(context, pId);
                                },
                                ...
                                style: TextButton.styleFrom(
                                  ...
                                  shape: RoundedRectangleBorder(
                                    ...
                                    side: BorderSide(color: AppTheme.vibrantGold.withOpacity(0.3)),
                                  ),
                                ),
                              ),
                            ),
